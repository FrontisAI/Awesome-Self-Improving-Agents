...
SOURCE_ROOT = REPO_ROOT.parent / "Agent_survey_Frontis"
MANUSCRIPT_ROOT = SOURCE_ROOT / "RL4LRM_Survey_0907"
AWESOME_README = REPO_ROOT / "README.md"
...
def parse_awesome_readme() -> list[dict]:
    if not AWESOME_README.exists():
        raise FileNotFoundError(f"Missing Awesome README: {AWESOME_README}")

    records: list[dict] = []
    current_category = ""
    seen_categories: set[str] = set()

    for line in AWESOME_README.read_text(encoding="utf-8").splitlines():
        if line.startswith("### "):
