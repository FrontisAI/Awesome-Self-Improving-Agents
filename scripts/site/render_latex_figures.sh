...
SITE_ROOT="${REPO_ROOT}"
SOURCE_ROOT="${MANUSCRIPT_ROOT:-${REPO_ROOT}/../Agent_survey_Frontis/RL4LRM_Survey_0907}"
...
render_one() {
  ...
  tmp_prefix="$(mktemp "${SITE_ROOT}/assets/render.XXXXXX")"
  "${PDFTOPPM}" -png -singlefile -r 220 "${SOURCE_ROOT}/${source_pdf}" "${tmp_prefix}"
  mv "${tmp_prefix}.png" "${SITE_ROOT}/${output_png}"
  rm -f "${tmp_prefix}"
}
...
render_one "figure/skill.pdf" "assets/skill-figure.png"
render_one "figure/memory.pdf" "assets/memory-figure.png"
render_one "figure/Environment_Runtime_Adaptation_perfect_editable.pdf" "assets/environment-figure.png"
